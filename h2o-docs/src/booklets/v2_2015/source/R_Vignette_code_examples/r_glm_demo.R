...
airlines_hex <- h2o.importFile(path = airlinesURL, destination_frame = "airlines_hex")
summary(airlines_hex)

# View quantiles and histograms
#high_na_columns <- h2o.ignoreColumns(data = airlines_hex)
quantile(x = airlines_hex$ArrDelay, na.rm = TRUE)
h2o.hist(airlines_hex$ArrDelay)

# Find number of flights by airport
originFlights <- h2o.group_by(data = airlines_hex, by = "Origin", nrow("Origin"),gb.control=list(na.methods="rm"))
originFlights.R <- as.data.frame(originFlights)

# Find number of flights per month
flightsByMonth <- h2o.group_by(data = airlines_hex, by = "Month", nrow("Month"),gb.control=list(na.methods="rm"))
flightsByMonth.R <- as.data.frame(flightsByMonth)

# Find months with the highest cancellation ratio
which(colnames(airlines_hex)=="Cancelled")
cancellationsByMonth <- h2o.group_by(data = airlines_hex, by = "Month", sum("Cancelled"),gb.control=list(na.methods="rm"))
...
airlines.split <- h2o.splitFrame(data = airlines_hex,ratios = 0.85)
