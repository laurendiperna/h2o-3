...
To upload a file in a directory local or remote to your H2O instance, which will be visible at all h2o nodes use {\texttt{h2o.importFile()}} (this is the recommended option). {\texttt{h2o.uploadFile()}}  uploads data local to your H2O instance as well as uploading data local to your R session, where the data only needs to be on the client. In the parentheses, specify the H2O reference object in R and the complete URL or normalized file path for the file.
...
\medskip
\begin{lstlisting}[style=R]
> prosPath <- "http://h2o-public-test-data.s3.amazonaws.com/smalldata/prostate/prostate.csv"
...
> australia.pca <- h2o.prcomp(training_frame = airlines_hex, transform = "STANDARDIZE",k = 3)
> australia.pca
Model Details:
==============
...
{\texttt{h2o.importFile}}: Import a file from the local or remote path and parse it.
